...
NION_GTK_TESTED_VERSION="$(_nion_manifest_read GTK_TESTED_VERSION)"
NION_WEBKITGTK_TESTED_VERSION="$(_nion_manifest_read WEBKITGTK_TESTED_VERSION)"
NION_GLIB_TESTED_VERSION="$(_nion_manifest_read GLIB_TESTED_VERSION)"
NION_GLIBC_FLOOR="$(_nion_manifest_read GLIBC_FLOOR)"
...
[[ "$NION_GLIBC_FLOOR" =~ ^[0-9]+\.[0-9]+(\.[0-9]+)?$ ]] || {
  echo "Invalid GLIBC_FLOOR in manifest: $NION_GLIBC_FLOOR" >&2; return 1 2>/dev/null || exit 1;
}
for _nion_dep_version in \
  "$NION_GTK_MIN_VERSION" "$NION_WEBKITGTK_MIN_VERSION" \
  "$NION_GTK_TESTED_VERSION" "$NION_WEBKITGTK_TESTED_VERSION" "$NION_GLIB_TESTED_VERSION"; do
  [[ "$_nion_dep_version" =~ ^[0-9]+\.[0-9]+(\.[0-9]+)?$ ]] || {
    echo "Invalid dependency version in manifest: $_nion_dep_version" >&2
    return 1 2>/dev/null || exit 1
  }
done
unset _nion_dep_version
...
export NION_GTK_MIN_VERSION NION_WEBKITGTK_MIN_VERSION
export NION_GTK_TESTED_VERSION NION_WEBKITGTK_TESTED_VERSION NION_GLIB_TESTED_VERSION NION_GLIBC_FLOOR
export NION_APPIMAGE_BASENAME NION_SOURCE_BASENAME
