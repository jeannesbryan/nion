...
/* Tab creation / current-tab / status / action-table operations that
 * main.c still owns (Phase 7, NiOn 2.0.0). main.c registers
 * implementations via nion_ui_set_callbacks() so this module never
 * calls into main.c directly. */
static NionUiCallbacks cb;
...
static void
nion_install_actions(NionApp *app)
{
    if (cb.install_actions)
        cb.install_actions(app);
}

void
nion_ui_set_callbacks(const NionUiCallbacks *callbacks)
{
    if (callbacks)
        cb = *callbacks;
}

/* Local forward declarations (definitions below are used before the
...
void nion_show_external_protocol_prompt(NionTab *tab,
                                               const gchar *uri,
                                               const gchar *scheme)
{
...
void nion_show_error_page(NionTab *tab,
...
gboolean on_permission_request(WebKitWebView *web_view,
                                      WebKitPermissionRequest *request,
                                      gpointer user_data)
{
    NionTab *tab = user_data;
...
void on_find_found(WebKitFindController *controller, guint match_count, gpointer user_data)
{
    (void)controller;
    NionTab *tab = user_data;
