...
- Blocks new web navigation when Tor is unavailable or the Tor child process fails (fail-closed).
...
NiOn 1.7.0 introduced a global Security Level in **Preferences**. The levels only tighten NiOn's existing hardening; selecting Standard never re-enables WebRTC, WebGL, WebAudio, JavaScript clipboard access, or automatic JavaScript popups.
...
Security Levels shipped as **Stage 1 — Security Levels** in the 1.7.0 development cycle and remain the current global hardening policy in NiOn 2.0.0. **Stage 2 — Escape Guards** (below) builds the external-protocol and popup boundary on top of the same release.
...
## Architecture & modularity (2.0.0)

NiOn 2.0.0 completes the decomposition of the former single-file `src/main.c` (~10.8k lines) into focused modules under `src/`, each with a matching `.h` interface. Modules are strictly layered: leaf logic never reaches back into the UI layer, and UI modules (`ui.c`, `app.c`) never call into `main.c`. Cross-module UI touchpoints (status bar, tab creation, controls, site-info refresh) are injected through small callback structs registered by `main.c`'s `on_activate`, keeping dependencies one-directional.

Key modules:

- `main.c` — entry point, `on_activate` callback coordinator, GAction table, webview-creation hub, security glue (content filter / onion-location / fail-closed policy).
- `app.c` — application lifecycle: directory/sandbox preparation, Tor-state coordination, close/shutdown flow, private-window lifecycle.
- `ui.c` — core chrome: window/toolbar builder, actions, dialogs, find/zoom/print, status/title/progress/control synchronizers.
- `tor-core.c` / `network.c` — bundled Tor subprocess lifecycle, dead-SOCKS fail-closed routing, download wiring.
- `session.c`, `tabs.c`, `webview.c`, `bookmarks.c`, `downloads.c`, `settings.c`, `site-data.c` — feature domains.
- `per-site.c`, `permission.c`, `privacy.c`, `content-filter.c` — leaf site/permission/privacy state.
- `navigation.c`, `util.c`, `types.h` — shared URI/policy predicates, utilities, and core types.

The Tor runtime and network layer are decoupled from the UI via a `NionTorCallbacks` surface (status / error / progress reporters registered at startup), so the Tor lifecycle has no direct UI dependency and fail-closed behavior can be reasoned about independently. See `docs/split-main-c-plan.md` for the full module map.
...
- A modular `src/` layout with one-directional dependencies keeps the security-sensitive core auditable.
